...
steiner::DynamicHsvSolver::DynamicHsvSolver(SteinerInstance* instance, node_id dualAscentLimit) :
    ...
    store_ = new DynamicHashSetLabelStore(instance_->getNumTerminals() - 1, instance->getGraph()->getMaxNode());
    ...
    if (instance->getGraph()->getNumEdges() <= dualAscentLimit_)
        heuristic_ = new DynamicDualAscentHeuristic(instance, root_, nTerminals_, instance_->getGraph()->getMaxNode());
    else
        heuristic_ = new DynamicMstHeuristic(instance, root_, nTerminals_);
...
SteinerResult* steiner::DynamicHsvSolver::solve() {
...
void steiner::DynamicHsvSolver::process_neighbors(node_id n, const dynamic_bitset<>* label, cost_id cost) {
...
void steiner::DynamicHsvSolver::process_labels(node_id n, const dynamic_bitset<>* label, cost_id cost) {
...
bool DynamicHsvSolver::prune(node_id n, cost_id cost, const dynamic_bitset<> *label) {
...
bool DynamicHsvSolver::prune(node_id n, cost_id cost, const dynamic_bitset<> *label1, const dynamic_bitset<>* label2,
...
void DynamicHsvSolver::prune_check_bound(node_id n, cost_id cost, const dynamic_bitset<> *label) {
...
unordered_map<dynamic_bitset<>, DynamicHsvSolver::PruneDistEntry>::iterator  DynamicHsvSolver::prune_compute_dist(const dynamic_bitset<> *label) {
...
cost_id DynamicHsvSolver::prune_combine(const dynamic_bitset<> *label1, const dynamic_bitset<> *label2, dynamic_bitset<> *combined) {
...
SteinerResult* DynamicHsvSolver::backTrack() {
    auto result = new SteinerResult(0, new Graph(), instance_->getGraph()->getReverseMapping(root_));
...
void DynamicHsvSolver::backTrackSub(node_id n, const dynamic_bitset<>* label, SteinerResult* result) {
